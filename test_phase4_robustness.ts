...
const mockTracker = {
  ...
  getRun: () => ({
    runId: "test-run",
    metrics: { cherishedFiles: [], releasedDebtCount: 0 },
  }),
  emitProgressUpdate: () => {},
} as any;
...
async function testPhase4() {
  ...
  console.log(
    `MarieResponse - getReasoning(): "${reasoning}" (Expected: "I should analyze the file first.")`,
  );
  if (reasoning !== "I should analyze the file first.")
    throw new Error("Reasoning extraction failed");
  ...
  const validResult = (processor as any).validate("write_to_file", {
    TargetFile: "test.ts",
    CodeContent: "hello",
    Overwrite: true,
  });
  console.log(
    `ToolProcessor - Valid validation: ${validResult === null ? "PASSED" : "FAILED (" + validResult + ")"}`,
  );
  if (validResult !== null) throw new Error("Valid tool rejected");

  // Test missing required field
  const invalidResult = (processor as any).validate("write_to_file", {
    TargetFile: "test.ts",
    CodeContent: "hello",
  });
  console.log(
    `ToolProcessor - Invalid validation (missing Overwrite): "${invalidResult}" (Expected: "Missing required fields: Overwrite")`,
  );
  if (!invalidResult || !invalidResult.includes("Overwrite"))
    throw new Error("Invalid tool accepted or wrong error message");
