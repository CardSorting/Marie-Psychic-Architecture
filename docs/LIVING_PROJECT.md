...
- **Intentional Naming**: We use `snake_case` for files (e.g., `message.tsx` -> `message_component.tsx` if strictly following, though we adapt to framework conventions like React Component naming `Message.tsx` while keeping logic files `snake_case`). _Note: We are currently evolving towards strict consistency._
...
_Reflect: Does your code feel heavy or light?_
