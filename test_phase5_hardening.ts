...
async function testPhase5() {
  ...
  console.log(
    `StringUtils - extractText: "${extracted}" (Expected: "Hello  World")`,
  );
  ...
  if (mergedA[0].type !== "text" || mergedA[1].type !== "tool_use")
    throw new Error("String merge failed");

  // Case B: Array content + tool
  const mergedB = mockMerge(
    [{ type: "text", text: "Hi" }],
    [{ name: "test_tool" }],
  );
  console.log(`Session Merge (Array) length: ${mergedB.length} (Expected: 2)`);
  if (mergedB[0].type !== "text" || mergedB[1].type !== "tool_use")
    throw new Error("Array merge failed");
