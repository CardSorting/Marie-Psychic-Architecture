...
import { getStringArg, getBooleanArg } from "../infrastructure/tools/ToolUtils.js";
import { NovelProductionService } from "../infrastructure/ai/narrative/NovelProductionService.js";
import { NarrativeAutomationServiceCLI, HardeningPhase } from "../services/NarrativeAutomationServiceCLI.js";
...
export function registerNovelTools(
    ...
    registry.register({
        ...
        description: "Ingest a lightnovel.md outline and initialize the production structure.",
        input_schema: {
            type: "object",
            properties: {
    ...
    registry.register({
        ...
        description: "End the current pass, lock files, and advance the chapter to the next phase.",
        input_schema: {
            type: "object",
            properties: {
                summary: { type: "string", description: "Summary of what was achieved in this pass." },
            },
            required: ["summary"],
        },
        execute: async (args) => {
    ...
    registry.register({
        ...
        description: "Run the Recursive Narrative Hardening Engine on a specific file.",
        input_schema: {
            type: "object",
            properties: {
                path: { type: "string", description: "Path to the markdown file to harden" },
                phase: {
                    type: "string",
                    enum: Object.values(HardeningPhase),
                    description: "The hardening phase to execute"
                },
                intent: { type: "string", description: "The artistic intent for this hardening pass" },
                recursive: { type: "boolean", description: "Whether to trigger the recursive hardening loop" },
            },
        ...
        execute: async (args) => {
            ...
            return await narrativeAutomation.performHardening(p, phase, intent, recursive);
        },
    });
