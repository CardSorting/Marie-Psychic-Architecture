...
export const PASS_ORDER: PassPhase[] = ["SKELETON", "FLESH", "NERVE", "SOUL", "CANON"];
...
export const PASS_PERSONA: Record<PassPhase, string> = {
    SKELETON: "MODE: SKELETON. You are the Architect. Build the bones — interfaces, types, wiring, config. No implementation logic. Structure only.",
    FLESH: "MODE: FLESH. You are the Craftsman. Fill the skeleton with domain logic and implementations. Do NOT touch interfaces or types from SKELETON.",
    NERVE: "MODE: NERVE. You are the Surgeon. Stress-test every joint — write tests, add error handling, validate edge cases. Do NOT rewrite logic from FLESH.",
    ...
    CANON: "MODE: HISTORIAN. The Volume is Canon. All passes complete. Protect the archive. Read-only.",
};
...
export class NovelProductionService {
    ...
    public async initialize() {
        try {
            const data = await fs.readFile(
                path.join(this.rootPath, NovelProductionService.NOVEL_FILE), "utf-8"
            );
            ...
            this.structure = {
                volumes: [{
                    ...
                    chapters: [{
                        ...
                        currentPass: "CANON",
                        completedPasses: ["SKELETON", "FLESH", "NERVE", "SOUL", "CANON"],
                        continuityLedger: [{
                            pass: "CANON",
                            summary: "Foundational prompts and entry point established.",
    ...
    public isPassLocked(filePath: string): { locked: boolean; lockedBy?: PassPhase } {
        const relative = path.relative(this.rootPath, filePath);
     ...
     */
    public async advancePass(summary: string): Promise<{ success: boolean; message: string }> {
        const activeChap = this.getActiveChapter();
        if (!activeChap) return { success: false, message: "No active chapter." };
        if (activeChap.currentPass === "CANON") return { success: false, message: "Chapter is already Canon." };

        // Gate: CritiqueService reviews the current pass
        const review = await this.critiqueService.reviewPass(activeChap, activeChap.currentPass, this.rootPath);
        if (!review.approved) {
            return { success: false, message: `Pass ${activeChap.currentPass} REJECTED. Score: ${review.score}. ${review.critique}` };
        }
    ...
    public getActiveContext(): string {
        ...
        const activeChap = this.getActiveChapter();
        const canonChaps = activeVol.chapters.filter(c => c.currentPass === "CANON");
...
${PASS_PERSONA.CANON}
        ...
        const ledgerSummary = activeChap.continuityLedger.length > 0
            ? activeChap.continuityLedger.map(e =>
