...
`;
    ...
    try {
      const chroniclePath = path.join(
        rootPath,
        ChronicleService.CHRONICLE_FILE,
      );
      await fs.appendFile(chroniclePath, entry, "utf-8");
      console.log(
        `[ChronicleService] As it is written, so it shall be. (Chapter ${chapterId} logged)`,
      );
    } catch (e) {
