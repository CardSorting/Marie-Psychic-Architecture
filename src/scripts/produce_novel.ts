...
async function main() {
    ...
    const novelStructurePath = path.join(workingDir, ".marie", "novel_structure.json");
    ...
    try {
        ...
        try {
            ...
            process.stdout.write(`✅ Initialized with ${chapters.length} chapters.\n`);
        } catch (err: any) {
            process.stderr.write(`❌ Failed to initialize directly: ${err.message}\n`);
            process.exit(1);
        }
    }
    ...
    while (true) {
        ...
        try {
            ...
            process.stderr.write(`❌ Failed to read novel structure: ${err.message}\n`);
            process.exit(1);
        }
        ...
        if (!activeVol) {
            process.stdout.write("🏁 No active draft volume found. Production complete?\n");
            break;
        }

        // Find the first non-Canon chapter
        const activeChap = activeVol.chapters.find((c: any) => c.currentPass !== "CANON");
        if (!activeChap) {
            process.stdout.write("🏁 All chapters are Canon in the active volume. Production complete!\n");
            break;
        }

        process.stdout.write(`\n───────────────────────────────────────────────────────────────────\n`);
        process.stdout.write(`📖 Chapter ${activeChap.id}: "${activeChap.title}"\n`);
        process.stdout.write(`🛠️ Pass: ${activeChap.currentPass} (${activeChap.completedPasses.length + 1}/5)\n`);
        process.stdout.write(`───────────────────────────────────────────────────────────────────\n\n`);
...
Apply code changes or write prose as required for this phase.
When your work for this pass is complete AND verified, call advance_novel_pass with a summary.`;
        ...
        try {
            ...
            const updatedVol = updatedStructure.volumes.find((v: any) => v.id === activeVol.id);
            const updatedChap = updatedVol.chapters.find((c: any) => c.id === activeChap.id);

            if (updatedChap.currentPass === activeChap.currentPass) {
                process.stdout.write(`⚠️ Pass did not advance. AI may require another turn or human intervention.\n`);
                // OPTIONAL: We could add a "nudge" message here to force advancement if we're sure work is done.
            } else {
                process.stdout.write(`✨ Advanced: ${activeChap.currentPass} -> ${updatedChap.currentPass}.\n`);
            }
        } catch (err: any) {
