...
suite("Marie Extension Live Tests", () => {
  ...
  test("webview HTML includes CSP and webview bundle references", async () => {
    ...
    assert.ok(
      typeof api.getWebviewHtml === "function",
      "Extension should expose getWebviewHtml()",
    );

    const html = api.getWebviewHtml();
    assert.ok(
      html.includes("Content-Security-Policy"),
      "Webview HTML should include CSP",
    );
    assert.ok(
      html.includes("main.js"),
      "Webview HTML should reference main.js bundle",
    );
    assert.ok(
      html.includes("main.css"),
      "Webview HTML should reference main.css bundle",
    );
  });
});
