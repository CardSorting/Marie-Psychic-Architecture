...
async function verifyDeepHardening() {
    ...
    try { await fs.mkdir(".marie"); } catch (e) { }
    ...
    try {
        ...
        await service.performHardening(TEST_FILE, HardeningPhase.CORE_EXPANSION, "Deepen traits", false);

        const expansionPrompt = capturedPrompts[0];
        if (expansionPrompt.includes("The Weaver") && expansionMagicCheck(expansionPrompt)) {
            console.log("✅ Context Injection Verified: Found 'The Weaver' and 'Magic needs blood'.");
        } else {
            throw new Error("Context Injection Failed! Prompt: " + expansionPrompt.substring(0, 100) + "...");
        }
        ...
        await service.performHardening(TEST_FILE, HardeningPhase.HOSTILE_ATTACK, "Brutalize", true);

        if (callCount === 3) {
            console.log("✅ Recursion Verified: 3 AI calls triggered (Attack, Defense, Audit).");
        } else {
