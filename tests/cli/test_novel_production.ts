...
Wakes in cathedral. First encounter.
    ...
    const narrativeAutomation = new NarrativeAutomationServiceCLI(workingDir, joyService);
    ...
    if (statusData.volumes[0].chapters.length < 2) throw new Error("Chapter initialization failed");
    ...
    console.log("Current Status Context:", statusResult?.substring(0, 100) + "...");
    ...
    try {
        const advanceResult = await advanceTool?.execute({ summary: "Test pass summary" });
