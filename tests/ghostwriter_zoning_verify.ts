...
import {
  AscensionState,
  AscensionDecree,
} from "../src/monolith/infrastructure/ai/core/MarieAscensionTypes.js";
...
async function verifyGhostwriter() {
  ...
  const decreeA: AscensionDecree = {
    ...
    profile: "balanced",
    ghostwriterMode: "EXPAND",
    raw: "",
  };
  (engine as any).state.lastDecree = decreeA;

  const zoningCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Intro",
  ...
  console.log(
    `- Zoning Check (Implementation in Core): ${zoningCheck.valid === false ? "PASS" : "FAIL"} (${zoningCheck.reason})`,
  );
  ...
  console.log(
    `- Drift Check (Variable Redefinition): ${driftCheck.valid === false ? "PASS" : "FAIL"} (${driftCheck.reason})`,
  );

  // 3. Test Anti-Collapse (Summarization in non-COMPRESS mode)
  const antiCollapseCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Intro",
  ...
  console.log(
    `- Anti-Collapse Check (Illegal Summary): ${antiCollapseCheck.valid === false ? "PASS" : "FAIL"} (${antiCollapseCheck.reason})`,
  );
  ...
  const povBleedCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Chapter1_Scene1",
      content:
        "Elias watched Lyra. Lyra felt a sudden pang of regret, knowing she had failed him.",
    },
    decreeA,
  );
  console.log(
    `- POV Check (Bleeding): ${povBleedCheck.valid === false ? "PASS" : "FAIL"} (${povBleedCheck.reason})`,
  );

  // 6. Test Causality Violation (Dead character speaking)
  const causalityCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Chapter1_Scene1",
  ...
  console.log(
    `- Causality Check (Dead Speaker): ${causalityCheck.valid === false ? "PASS" : "FAIL"} (${causalityCheck.reason})`,
  );

  // 7. Test Thematic Zone Violation (Dialogue in Z0)
  const thematicCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Chapter1_Theme",
  ...
  console.log(
    `- Thematic Check (Dialogue in Z0): ${thematicCheck.valid === false ? "PASS" : "FAIL"} (${thematicCheck.reason})`,
  );
  ...
  console.log(
    `- Novel Drift Check (New Character): ${novelDriftCheck.valid === false ? "PASS" : "FAIL"} (${novelDriftCheck.reason})`,
  );
  ...
  console.log(
    `- Genesis Override Check: ${genesisCheck.valid === true ? "PASS" : "FAIL"}`,
  );
  ...
  const intimacyViolationCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Chapter1_Scene1",
      content:
        "Elias looked at Marcus and said, 'I've missed you, my dear friend.'",
    },
    decreeA,
  );
  console.log(
    `- Intimacy Check (Affectionate Register): ${intimacyViolationCheck.valid === false ? "PASS" : "FAIL"} (${intimacyViolationCheck.reason})`,
  );

  // 11. Test Intimacy Violation (Physical contact despite low intimacy)
  const intimacyPhysicalCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Chapter1_Scene1",
  ...
  console.log(
    `- Intimacy Check (Illegal Contact): ${intimacyPhysicalCheck.valid === false ? "PASS" : "FAIL"} (${intimacyPhysicalCheck.reason})`,
  );

  // Visceral / Cognitive Setup
  const elias = state.ghostwriterMemory.characterBible.find(
    (c) => c.name === "Elias",
  )!;
  elias.somatic = { pulse: 140, breath: "STACCATO", tension: 0.9 };
  elias.biases = [
    { name: "Grief", intensity: 0.9, distortionRule: "Cold light" },
  ];

  // 12. Test Somatic Violation (Fluid prose during high tension)
  const somaticViolationCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Chapter1_Scene1",
      content:
        "Elias watched the sun rise slowly over the hills of the valley, remembering the long days of his childhood when everything felt simpler and the world was filled with golden light that seemed to stretch on forever.",
    },
    decreeA,
  );
  console.log(
    `- Somatic Check (Fluid Prose during Tension): ${somaticViolationCheck.valid === false ? "PASS" : "FAIL"} (${somaticViolationCheck.reason})`,
  );

  // 13. Test Refraction Violation (Cheerful cues during extreme grief)
  const refractionViolationCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Chapter1_Scene1",
  ...
  console.log(
    `- Refraction Check (Grief Distortion): ${refractionViolationCheck.valid === false ? "PASS" : "FAIL"} (${refractionViolationCheck.reason})`,
  );

  // 14. Test Motif Refraction (Lazy Bridge)
  const lazyMotifCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Chapter1_Scene1",
  ...
  console.log(
    `- Motif Refraction Check (Lazy Bridge): ${lazyMotifCheck.valid === false ? "PASS" : "FAIL"} (${lazyMotifCheck.reason})`,
  );

  // 15. Test Motif Refraction (Voice Clash)
  const voiceClashCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Chapter1_Scene1",
  ...
  console.log(
    `- Motif Refraction Check (Voice Clash): ${voiceClashCheck.valid === false ? "PASS" : "FAIL"} (${voiceClashCheck.reason})`,
  );

  // 16. Test Chrono Violation (Low Density during Dilation)
  const chronoCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Chapter1_Scene1",
  ...
  console.log(
    `- Chrono Check (Time Dilation): ${chronoCheck.valid === false ? "PASS" : "FAIL"} (${chronoCheck.reason})`,
  );
  ...
  const synesthesiaCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Chapter1_Scene1",
      content:
        "He stood in the silence and felt the heavy scent of the gold light washing over his skin like a physical weight.",
    },
    decreeA,
  );
  console.log(
    `- Synesthesia Check (Unearned): ${synesthesiaCheck.valid === false ? "PASS" : "FAIL"} (${synesthesiaCheck.reason})`,
  );

  // 18. Test Motif Bleeding (Low Intimacy)
  const bleedCheck = (engine as any).validateZoning(
    "write_to_file",
    {
      path: "Chapter1_Scene1",
      content:
        "Marcus moved with the salt and ebb of a desert storm, his every gesture mirroring the tide.",
    },
    decreeA,
  );
  console.log(
    `- Motif Bleed Check (Low Intimacy): ${bleedCheck.valid === false ? "PASS" : "FAIL"} (${bleedCheck.reason})`,
  );
