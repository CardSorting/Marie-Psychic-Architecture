...
async function verifyHardening() {
  ...
  const mockProvider = {
    createMessage: async (params: any) => {
      console.log(
        `🤖 AI Called with prompt length: ${params.messages[0].content.length}`,
      );
      return {
        content:
          "AI Generated Content for " +
          params.messages[0].content.substring(0, 20) +
          "...",
        role: "assistant",
      };
    },
  ...
  try {
    ...
    const result = await service.performHardening(
      testFile,
      HardeningPhase.ATOMIC_EXTRACTION,
      "Extract claims",
    );
